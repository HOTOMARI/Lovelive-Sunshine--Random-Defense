...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdParam, int nCmdShow) {
	...
	// 윈도우 생성
	hWnd = CreateWindow(lpszClass, "Lovelive! Sunshine! Random Defense", WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, 0, 0, width, 807, NULL, (HMENU)NULL, hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT IMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static CImage Grass1, Sand1, Tower, Monster, Effect[5], Area, Bullet, Map[3], Boss, Skill, Cutin[9], Atkeffect[3][9], System[4], Subskill[9],HowtoP[8];
	PAINTSTRUCT ps;
	BITMAP bmp;
	...
	case WM_PAINT:
		...
		// 더블버퍼링
		if (hBit == NULL) {
			hBit = CreateCompatibleBitmap(hdc, window_W, window_H);
		}
		...
		Rectangle(mem1dc, 0, 0, window_W, window_H);
		...
		if (mod.gamemode) {
			...
			for (i = 0; i < 12; ++i) {
				for (j = 0; j < 12; ++j) {
					if (map[i][j].tower.ur_skillchk) {
						...
						case 0:
							if (map[i][j].tower.target != NULL && map[i][j].tower.target < info.monster_num && map[i][j].tower.skillcool > 700) {
			...
			if (mod.boss == 1) {
				for (i = 0; i < info.monster_num; ++i) {
					...
					if (monster[i].boss.skill == false) {
						hBrush = CreateSolidBrush(RGB(0, 255, 0));
						(HBRUSH)SelectObject(mem1dc, hBrush);
				...
				for (i = 0; i < 12; ++i) {
					for (j = 0; j < 12; ++j) {
						if (map[mouse.sel_y][mouse.sel_x].tower.rank == map[i][j].tower.rank
							&& map[mouse.sel_y][mouse.sel_x].tower.kind == map[i][j].tower.kind && map[i][j].install && info.combi_tower == 0) {
							Tower.Draw(mem1dc, map[i][j].x, map[i][j].y, info.tile_W, info.tile_H,
			...
			// 스킬 효과 출력
				...
				case 9:
					for (i = 0; i < 10; ++i) {
						if (only_kanan[i].tile_x > 0) {
							Skill.AlphaBlend(mem1dc, only_kanan[i].tile_x * 64, only_kanan[i].tile_y * 64, 64, 64, 0, 256, 256, 256, 255 - 5 * skill.skill_timer);
						}
					}
					break;
				}
		...
		// 더블버퍼링
		BitBlt(hdc, 0, 0, window_W, window_H, mem1dc, 0, 0, SRCCOPY);
	...
	case WM_LBUTTONDOWN:
		if (mod.gamemode) {
			...
			if (mod.install == 1 && map[mouse.sel_y][mouse.sel_x].install == false && map[mouse.sel_y][mouse.sel_x].kind == 1 && info.money >= 5) {
				...
				//map[mouse.sel_y][mouse.sel_x].tower.kind = 2;
	...
	case WM_TIMER:
		...
		if (mod.gamemode) {
			...
			if (skill.activate == false || (skill.type == 2 || skill.type == 5 || skill.type == 6 || skill.type == 8) && skill.play_cutin == false) {
				...
				if (mod.breaktime == 0 && mod.boss == 0) {
					// 몬스터 생성 하면서 정의
					if (timer.frame_tmr >= 60 && info.count_mon < 20) {
				...
				if ((mod.boss == true && monster[0].boss.skill == true)) {
					shotdown += 768 / 50;
				}
				...
				if (mod.boss == false || ((mod.boss == true && monster[0].boss.skill == false) || info.now_wave / 10 != 3)) {
					...
					for (j = 0; j < 12; ++j) {
						for (k = 0; k < 12; ++k) {
							if (map[j][k].install) {
								...
								if (info.monster_num == 0) {
									map[j][k].tower.target = NULL;
								}
					...
					for (j = 0; j < 12; ++j) {
						for (k = 0; k < 12; ++k) {
							if (map[j][k].install) {
								if (map[j][k].tower.target < info.monster_num) {
									if (inrange(map[j][k].tower.range, map[j][k].x + info.tile_W / 2, map[j][k].y + info.tile_H / 2,
										...
										bullet[info.bullet_num - 1].atk = map[j][k].tower.atk;
						...
						if (bullet[i].target_lose == false) {
							...
							if (bullet[i].hit) {
								...
								if (monster[bullet[i].target].boss.skill == false && monster[bullet[i].target].boss.boss && monster[bullet[i].target].kind == 2) {		// 2번째 보스 때릴때마다 스킬쿨감소
									monster[bullet[i].target].boss.skillcool -= bullet[i].atk / 20;
					...
					for (i = 0; i < info.bullet_num; ++i) {
						if (bullet[i].target_lose) {
							RECT bul = { bullet[i].x, bullet[i].y, bullet[i].x + info.object_W / 2, bullet[i].y + info.object_H / 2 };
							RECT mob = { bullet[i].target_x, bullet[i].target_y, bullet[i].target_x + info.tile_W / 2, bullet[i].target_y + info.tile_H / 2 };
							bullet[i].x += bullet[i].mx * bullet[i].speed;
							bullet[i].y += bullet[i].my * bullet[i].speed;
							bullet[i].frame++;
							bullet[i].frame %= bullet[i].maxframe;
				...
				for (i = 0; i < info.monster_num; ++i) {
					if (monster[i].hp <= 0) {
						...
						if (monster[i].death > 15) {
							info.kill_count += 1;		// 킬 카운트
							for (j = 0; j < info.bullet_num; ++j) {
								if (bullet[j].target == i) {		// 죽었으니 총알타겟 없앰
									bullet[j].target_lose = true;
								}
								if (bullet[j].target > i) {			// 총알을 배열로 만들어서 하는짓
									bullet[j].target -= 1;
								}
							}
							if (skill.degree < 360) {
			...
			if (skill.activate&&skill.play_cutin == false) {
				...
				case 9:
					...
					tmp_y = rand() % 11 + 1;
					while (skill.skill_timer < 10) {
						if (map[tmp_y][tmp_x].kind != 1) {
							only_kanan[skill.skill_timer].tile_x = tmp_x;
							only_kanan[skill.skill_timer].tile_y = tmp_y;
							only_kanan[skill.skill_timer].type = 9;
						}
						switch (rand() % 2)
						{
						case 0:
					...
					if (skill.kanan_timer == 64) {
						for (i = 0; i < 10; ++i) {
							for (j = 0; j < info.monster_num; ++j) {
								skill_damage(only_kanan[i], &monster[j], info.now_wave);
							}
						}
					}
...
LRESULT CALLBACK ChildProc_1(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	HDC hDC, memDC, bitmapDC;
	PAINTSTRUCT ps;
	HPEN hPen, oldPen;
	HBRUSH hBrush, oldBrush;
	static HBITMAP hBitmap;
	static CImage Menu, Card;
	static RECT rt;

	// 메뉴 배경 32*154 가로 183 시작 세로 154 시작
	// 메뉴 버튼 크기 152*77
	// 스킬 버튼 크기 64*64	가로 152 시작
	// 스킬 아이콘 크기 32*32		가로 216 시작
	...
	case WM_CREATE:
		...
		// 버튼 위치 설정
		for (int i = 0; i < 3; ++i) {
			buttons[i].x = 0, buttons[i].y = i * 77;
			buttons[i].width = 152, buttons[i].height = 77;
		}
		Button tmp = buttons[1];
		buttons[1] = buttons[2];
		buttons[2] = tmp;
		buttons[3].x = 580, buttons[3].y = 30;
	...
	case WM_PAINT:
		...
		{
			// 메뉴 배경
			Menu.Draw(memDC, 0, 0, 32, rt.bottom, 183, 154, 32, 154);
			Menu.Draw(memDC, 32, 0, rt.right - 64, rt.bottom, 183 + 32 * 1, 154, 32, 154);
			Menu.Draw(memDC, rt.right - 32, 0, 32, rt.bottom, 183 + 32 * 2, 154, 32, 154);
			// 인포메이션 윈도우
			// 현재 기준 시작좌표 155,20		크기 518*171
			Menu.Draw(memDC, 155, 20, 32, rt.bottom - 40, 184 + 32 * 3, 154, 32, 154);
			Menu.Draw(memDC, 187, 20, rt.right - 437, rt.bottom - 40, 184 + 32 * 4, 154, 32, 154);
			Menu.Draw(memDC, rt.right - 250, 20, 32, rt.bottom - 40, 184 + 32 * 5, 154, 32, 154);
			{
				char tmp[4];
				// 웨이브
				Menu.Draw(memDC, 180, 171, 60, 30, 184, 109, 96, 40);
				if (info.now_wave / 10 != 0)
					Menu.Draw(memDC, 242, 175, 25, 25, number[info.now_wave / 10].x, number[info.now_wave / 10].y, number[info.now_wave / 10].width, number[info.now_wave / 10].height);
				Menu.Draw(memDC, 262, 175, 25, 25, number[info.now_wave % 10].x, number[info.now_wave % 10].y, number[info.now_wave % 10].width, number[info.now_wave % 10].height);
				// 생명
				Menu.Draw(memDC, 320, 178, 20, 20, 178, 64, 13, 13);
				if (info.remain_life > 0) {
					if (info.remain_life / 10 != 0)
						Menu.Draw(memDC, 343, 175, 25, 25, number[info.remain_life / 10].x, number[info.remain_life / 10].y, number[info.remain_life / 10].width, number[info.remain_life / 10].height);
					Menu.Draw(memDC, 363, 175, 25, 25, number[info.remain_life % 10].x, number[info.remain_life % 10].y, number[info.remain_life % 10].width, number[info.remain_life % 10].height);
				}
				// 재화
				Menu.Draw(memDC, 440, 178, 20, 20, 165, 64, 13, 13);
				if (info.money >= 100)
					Menu.Draw(memDC, 462, 175, 25, 25, number[info.money / 100].x, number[info.money / 100].y, number[info.money / 100].width, number[info.money / 100].height);
				if (info.money >= 10)
					Menu.Draw(memDC, 482, 175, 25, 25, number[info.money % 100 / 10].x, number[info.money % 100 / 10].y, number[info.money % 100 / 10].width, number[info.money % 100 / 10].height);
				Menu.Draw(memDC, 502, 175, 25, 25, number[info.money % 10].x, number[info.money % 10].y, number[info.money % 10].width, number[info.money % 10].height);
			}
			// 선택한 타워 정보
			// 모드 출력
			if (mod.view_info == 1) {
				...
				{
				case 0:
					Menu.Draw(memDC, 180, 30, 40, 40, 248, 32, 32, 32);
					break;
				case 1:
					Menu.Draw(memDC, 180, 30, 40, 40, 280, 32, 32, 32);
					break;
				case 2:
					Menu.Draw(memDC, 180, 30, 40, 40, 312, 32, 32, 32);
					break;
				case 3:
					Menu.Draw(memDC, 180, 30, 40, 40, 344, 32, 32, 32);
					break;
				}
				// 이름
				...
				{
				case 0:
					Menu.Draw(memDC, 222, 40, 89, 23, 376, 109, 89, 23);
					break;
				case 1:
					Menu.Draw(memDC, 222, 40, 133, 23, 376, 133, 133, 23);
					break;
				case 2:
					Menu.Draw(memDC, 222, 40, 87, 23, 376, 156, 87, 23);
					break;
				case 3:
					Menu.Draw(memDC, 222, 40, 118, 23, 376, 181, 118, 23);
					break;
				case 4:
					if (rand() % 2 == 0)
						Menu.Draw(memDC, 222, 40, 104, 23, 376, 206, 104, 23);
					else Menu.Draw(memDC, 222, 40, 103, 23, 376, 230, 103, 23);
					break;
				case 5:
					Menu.Draw(memDC, 222, 40, 118, 23, 376, 254, 118, 23);
					break;
				case 6:
					Menu.Draw(memDC, 222, 40, 104, 23, 376, 279, 104, 23);
					break;
				case 7:
					Menu.Draw(memDC, 222, 40, 103, 23, 376, 303, 103, 23);
					break;
				case 8:
					Menu.Draw(memDC, 222, 40, 103, 23, 376, 327, 103, 23);
					break;
				}
				// 얼굴
				switch (map[mouse.save_y][mouse.save_x].tower.kind)
				...
				case 0:
					...
					{
					case 3:
						Card.Draw(memDC, 430, 30, 96, 135, 0, 0, 96, 135);
						break;
					case 2:
						Card.Draw(memDC, 430, 30, 96, 135, 0, 135, 96, 135);
						break;
					case 1:
						Card.Draw(memDC, 430, 30, 96, 135, 0, 270, 96, 135);
						break;
					case 0:
						Card.Draw(memDC, 430, 30, 96, 135, 0, 405, 96, 135);
						break;
					}
					break;
				case 1:
					...
					{
					case 3:
						Card.Draw(memDC, 430, 30, 96, 135, 96, 0, 96, 135);
						break;
					case 2:
						Card.Draw(memDC, 430, 30, 96, 135, 96, 135, 96, 135);
						break;
					case 1:
						Card.Draw(memDC, 430, 30, 96, 135, 96, 270, 96, 135);
						break;
					case 0:
						Card.Draw(memDC, 430, 30, 96, 135, 96, 405, 96, 135);
						break;
					}
					break;
				case 2:
					...
					{
					case 3:
						Card.Draw(memDC, 430, 30, 96, 135, 192, 0, 96, 135);
						break;
					case 2:
						Card.Draw(memDC, 430, 30, 96, 135, 192, 135, 96, 135);
						break;
					case 1:
						Card.Draw(memDC, 430, 30, 96, 135, 192, 270, 96, 135);
						break;
					case 0:
						Card.Draw(memDC, 430, 30, 96, 135, 192, 405, 96, 135);
						break;
					}
					break;
				case 3:
					...
					{
					case 3:
						Card.Draw(memDC, 430, 30, 96, 135, 288, 0, 96, 135);
						break;
					case 2:
						Card.Draw(memDC, 430, 30, 96, 135, 288, 135, 96, 135);
						break;
					case 1:
						Card.Draw(memDC, 430, 30, 96, 135, 288, 270, 96, 135);
						break;
					case 0:
						Card.Draw(memDC, 430, 30, 96, 135, 288, 405, 96, 135);
						break;
					}
					break;
				case 4:
					...
					{
					case 3:
						Card.Draw(memDC, 430, 30, 96, 135, 384, 0, 96, 135);
						break;
					case 2:
						Card.Draw(memDC, 430, 30, 96, 135, 384, 135, 96, 135);
						break;
					case 1:
						Card.Draw(memDC, 430, 30, 96, 135, 384, 270, 96, 135);
						break;
					case 0:
						Card.Draw(memDC, 430, 30, 96, 135, 384, 405, 96, 135);
						break;
					}
					break;
				case 5:
					...
					{
					case 3:
						Card.Draw(memDC, 430, 30, 96, 135, 480, 0, 96, 135);
						break;
					case 2:
						Card.Draw(memDC, 430, 30, 96, 135, 480, 135, 96, 135);
						break;
					case 1:
						Card.Draw(memDC, 430, 30, 96, 135, 480, 270, 96, 135);
						break;
					case 0:
						Card.Draw(memDC, 430, 30, 96, 135, 480, 405, 96, 135);
						break;
					}
					break;
				case 6:
					...
					{
					case 3:
						Card.Draw(memDC, 430, 30, 96, 135, 576, 0, 96, 135);
						break;
					case 2:
						Card.Draw(memDC, 430, 30, 96, 135, 576, 135, 96, 135);
						break;
					case 1:
						Card.Draw(memDC, 430, 30, 96, 135, 576, 270, 96, 135);
						break;
					case 0:
						Card.Draw(memDC, 430, 30, 96, 135, 576, 405, 96, 135);
						break;
					}
					break;
				case 7:
					...
					{
					case 3:
						Card.Draw(memDC, 430, 30, 96, 135, 672, 0, 96, 135);
						break;
					case 2:
						Card.Draw(memDC, 430, 30, 96, 135, 672, 135, 96, 135);
						break;
					case 1:
						Card.Draw(memDC, 430, 30, 96, 135, 672, 270, 96, 135);
						break;
					case 0:
						Card.Draw(memDC, 430, 30, 96, 135, 672, 405, 96, 135);
						break;
					}
					break;
				case 8:
					...
					{
					case 3:
						Card.Draw(memDC, 430, 30, 96, 135, 768, 0, 96, 135);
						break;
					case 2:
						Card.Draw(memDC, 430, 30, 96, 135, 768, 135, 96, 135);
						break;
					case 1:
						Card.Draw(memDC, 430, 30, 96, 135, 768, 270, 96, 135);
						break;
					case 0:
						Card.Draw(memDC, 430, 30, 96, 135, 768, 405, 96, 135);
						break;
					}
					break;
				}
				...
				Menu.Draw(memDC, 230, 50, 240, 70, 269, 308, 104, 29);
			}
			else if (mod.install == 1) {
				Menu.Draw(memDC, 250, 50, 200, 80, 184, 308, 83, 29);
			}
			else if (mod.sell == 1) {
				Menu.Draw(memDC, 250, 50, 200, 80, 184, 337, 83, 29);
			}
			// 메세지
			SetBkMode(memDC, TRANSPARENT);
			if (upgrade_status == 1) {
				wsprintf(upgrade_message, "+%d Upgrade Complete", info.upgrade_stack[upgrade_kind]);
				TextOut(memDC, 270, 145, upgrade_message, strlen(upgrade_message));
			}
			else if (upgrade_status == 2) {
				TextOut(memDC, 300, 145, "Upgrade Fail", 12);
			}

			// 업그레이드 버튼
			for (int i = 0; i < 3; ++i) {
				if (buttons[i].on_mouse)
					Menu.Draw(memDC, buttons[i].x, buttons[i].y, buttons[i].width, buttons[i].height, buttons[i].x, i * 154 + 77, buttons[i].width, buttons[i].height);
				else Menu.Draw(memDC, buttons[i].x, buttons[i].y, buttons[i].width, buttons[i].height, buttons[i].x, i * 154, buttons[i].width, buttons[i].height);
			}
			// 스킬 버튼
			Ellipse(memDC, buttons[3].x - 10, buttons[3].y - 10, buttons[3].x + 186, buttons[3].y + 186);
			hPen = CreatePen(PS_SOLID, 2, RGB(0, 255, 0));
			oldPen = (HPEN)SelectObject(memDC, hPen);
			for (int i = 0; i < skill.degree; i += 2) {
				MoveToEx(memDC, 658, 108, NULL);
				LineTo(memDC, 98 * cos(get_radian(i - 89)) + 668, 98 * sin(get_radian(i - 89)) + 118);
			}
			SelectObject(memDC, oldPen);
			DeleteObject(hPen);

			Menu.Draw(memDC, buttons[3].x, buttons[3].y, buttons[3].width, buttons[3].height, 152, 0, 64, 64);
			if (skill.type == 0)
				Menu.Draw(memDC, 618, 70, 100, 100, 216, 32, 32, 32);
			else
				Menu.Draw(memDC, 618, 70, 100, 100, 216 + 32 * (skill.type - 1), 0, 32, 32);
		}
